...
namespace TagLib {
  ...
  %typemap(dtype) String "string"
  %typemap(imtype) String "char*"
  %typemap(ctype) String "char*"
  %typemap(in) String {$1 = TagLib::String($input, TagLib::String::UTF8);}
  %pragma(d) globalproxyimports = "import std.conv;";
  %pragma(d) globalproxyimports = "import std.string;";
  %pragma(d) globalproxyimports = "import core.stdc.stdlib;";
  %typemap(out) String
  {
      std::string buffer = $1.to8Bit(true);
      $result = (char*)malloc(buffer.size() + 1);
      strcpy($result, buffer.c_str());
  }
  %typemap(din) String "cast(char*)toStringz($dinput)"
  %typemap(dout) String
  {
      char* cstr = $imcall;
      string tmp;
  ...
  %typemap(dtype) const String & "string"
  %typemap(imtype) const String & "char*"
  %typemap(ctype) const String & "char*"
  %typemap(in) const String & {$1 = &TagLib::String($input, TagLib::String::UTF8);}
  ...
  %typemap(din) const String & "cast(char*)toStringz($dinput)"
  %typemap(dout) const String &
  {
      char* cstr = $imcall;
      string tmp;
      try
