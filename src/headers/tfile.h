...
namespace TagLib {
  ...
  %typemap(dtype) FileName "string"
  %typemap(imtype) FileName & "const(char)*"
  %typemap(ctype) FileName "const char*"
  %typemap(in) FileName & {$1 = FileName($input);}
  %typemap(out) FileName {
/*#ifdef _WIN32
      $result = (char*)malloc($1.size()+1);
      strcpy($result, $1);
#else*/
      $result = $1;
/*#endif*/
}
  %typemap(din) FileName "std.string.toStringz($dinput)"
  %typemap(dout) FileName
  {
      return to!string($imcall);
  }
  %ignore "FileName";
#ifdef _WIN32
